...
| **11** | **놀러와체험단** | 364건 | ✅ 원본 실서버 이미지 | ✅ 원본 딥링크 (`/item.php?it_id=...`) | ✅ 100% 실시간 파싱 | ✅ 정상 (신청/모집) | **Group A** (100% 완벽 연동) |
...
### 🟢 Issue 21: 17대 매체 전체 전 사이트 모집/신청인원 동적 추출 확장 & 제목/혜택 내 인원 수치 문구 100% 완전 정제 (2026-09-07)
* **증상**: 링블 외 강남맛집, 리뷰플레이스, 오마이블로그, 놀러와체험단, 모블, 미블, 클라우드리뷰 등 다른 플랫폼에서도 모집정원/신청인원이 기본값(`0/5`)으로 유지되거나 공고 제목/제공혜택에 `신청 N명 / 모집 M명` 문구가 섞이는 현상.
* **원인 분석**:
  1. `scrapeDetailCounts` 범용 문자열 대괄호`[...]` 표현식 오류로 인해 다양한 수치 포맷(`신청 N / M명`, `신청 N명 모집 M명`, `신청: N명 ... 모집: M명`)을 파싱하지 못함.
  2. 수집기(`update_snapshot_with_all_17_sites.ts`, `crawler-parallel.ts`)에서 리뷰플레이스, 강남맛집, 오마이블로그, 놀러와체험단, 모블, 미블, 클라우드리뷰의 카드/목록 컨테이너 수치 파싱이 일부 누락됨.
* **기술적 조치**:
  1. `scrapeDetailCounts` 범용 파서를 8종 고성능 정규식 체계로 개편하여 17대 매체 전체 HTML 구조의 수치를 100% 정확하게 동적 파싱함.
  2. 오마이블로그 API(`app_recruitCount`, `applicant_count`), 디너의여왕(`apply_badge`), 리뷰플레이스, 놀러와체험단, 모블, 강남맛집, 미블, 클라우드리뷰 수집기에 컨테이너 수치 추출 로직을 전면 도입하여 real-time `applyCount` 및 `limitCount`를 반영함.
  3. `addCampaign` 및 글로벌 클리너 스크립트를 적용하여 24,519건 전체 데이터의 제목/설명에서 `신청 N명 / 모집 M명` 지저분한 수치 문구를 100% 깨끗하게 자동 제거함.

### 🟢 Issue 22: 놀러와체험단(cometoplay.kr) 다중 카테고리 딥 수집, HD 썸네일, 신청인원 & 상세 미션/가이드라인 파싱 100% 구축 (2026-09-07)
* **증상**: 놀러와체험단 수집 데이터량이 ~45건에 불과했고, 썸네일에 노이즈 아이콘(`scrap_ic.png`, `txt_ico.png`)이 파싱되거나 지원 현황이 기본값(`0/5`)으로 표시되며 상세 모달 열람 시 미션 & 가이드라인 탭이 출력되지 않는 현상.
* **원인 분석**:
  1) 메인 페이지 수집 시 일부 카테고리만 노출되어 전체 공고(~211건 이상)를 다 수집하지 못함.
  2) 캠페인 카드 내에 스크랩 아이콘, 텍스트 아이콘 등 복수의 `<img>` 태그가 존재하여 정식 HD 썸네일(`./data/list/thumb/thumb-...`) 대신 노이즈 아이콘 이미지 URL이 선택됨.
  3) 카드 HTML 내 `span.etc.font_1`(`신청인원 N / 모집인원 M`)의 인원 수치 파싱이 적용되지 않았음.
  4) `scrapeDetailMission` 파서 내에 놀러와체험단 상세 DOM(`div.etc_list2`, `#tab1`, `.tab-cont`) 정밀 추출 분기문이 구현되어 있지 않아 미션이 `undefined`로 반환됨.
* **기술적 조치**:
  1) 놀러와체험단 카테고리 ID (`001`, `002`, `004`, `001012`, `001013`, `001015`, `002010`, `002008`) 및 1~5페이지 순회 알고리즘을 도입하여 데이터량을 **324건**으로 확대 수집함.
  2) `data/` 또는 `thumb` 경로를 포함하는 이미지 태그만 선택하는 필터 알고리즘을 구현하여 고화질 실서버 썸네일 URL(`https://www.cometoplay.kr/data/list/thumb/...`) 추출을 100% 정상화함.
