...
### 🟢 Issue 20: 링블(Ringble) 모집/신청 인원 수치 고정(0/5) 버그 파싱 정밀화 및 동적 추출 (2026-09-07)
* **증상**: 링블 공고 목록 및 상세에서 모집정원/신청인원이 실제 수치(예: 신청 19 / 모집 3)와 상관없이 기본값(`지원현황 0/5`)으로 고정 노출되는 현상.
* **원인 분석**:
  1. `scrapeDetailCounts` 내 범용 수치 추출 정규식 `[\/|명\s*모집|모집]`의 대괄호`[...]` 표현 오류로 인해 링블 상세 HTML의 `신청 19 / 모집 3` 문구를 파싱하지 못함.
  2. 링블 목록 수집 스크립트(`update_snapshot_with_all_17_sites.ts` 및 `crawler-parallel.ts`)에서 링블 공고 HTML 테이블 컨테이너 내 `신청 N / 모집 M` 수치를 추출하여 `applyCount` / `limitCount` 필드에 할당하는 로직이 누락되어 기본값 0/5가 적용됨.
* **기술적 조치**:
  1. `scrapeDetailCounts`에 링블 전용 스크레이퍼 조건문(`siteLower.includes('링블')`)을 추가하여 링블 상세 페이지 text에서 신청/모집 정원 동적 파싱 구현.
  2. 링블 목록 파서에서 `$(el).parents('table').first().text()` 기반 정규식 `신청\s*([\d,]+)\s*\/\s*모집\s*([\d,]+)` 파싱을 추가하여 460여 건 전체 링블 공고에 대해 real-time `applyCount` 및 `limitCount`를 정확하게 추출 및 반영함.
