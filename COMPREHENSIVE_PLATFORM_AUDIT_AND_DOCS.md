...
### 🟢 Issue 18: 제공혜택 인원 수치(신청 0/ 모집 5) 제거 및 인스타그램 뱃지 오분류 100% 교정 (2026-09-07)
* **증상 1**: 제공혜택 영역에 `신청 0/ 모집 5`, `신청 0 / 모집 5` 등의 인원 현황 문구가 제거되지 않고 출력되는 현상.
* **증상 2**: 제목/설명에 `[릴스]`, `인스타`가 포함된 인스타그램 캠페인의 썸네일에 네이버 블로그 마크가 표시되는 현상.
* **원인 분석**:
  1. 기존 `sanitizeCampaignText` 정규식이 `명`(예: `신청 0명 / 모집 5명`) 단어를 필수 조건으로 요구하여, `명` 자가 생략된 `신청 0/ 모집 5` 및 `신청 0 / 모집 5` 패턴을 걸러내지 못함.
  2. `detectPlatform` 함수 호출 시 기존 `c.platform || c.description` 전달 구조로 인해, 기존 DB/JSON에 `platform: 'blog'`가 들어있던 인스타그램 캠페인들이 `rawPlatformText`로 `'blog'`를 받아 400여 건 이상 블로그로 오분류됨.
* **조치 및 해결**:
  1. `sanitizeCampaignText` 정규식을 개편하여 `명`, 콜론, 슬래시, 괄호 여부와 상관없이 모든 형태의 신청/모집 수치 문구를 100% 완벽 정제함.
  2. `detectPlatform` 엔진을 최우선 SNS 탐지 알고리즘으로 개편하고, 22,262건 전체 데이터에 마이그레이션 스크립트(`reclassify_and_clean_all.ts`)를 실행하여 3,527건의 인스타그램/유튜브/클립/쿠팡 매체를 정밀 재분류함.
