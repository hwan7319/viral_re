import { fetchBlogStats } from './blog-stats';
...
export function parseSearchAdVolume(val: any): number {
  if (typeof val === 'number') return Number.isFinite(val) && val >= 0 ? val : NaN;
  if (typeof val === 'string') {
    if (val.includes('<')) return NaN; // Censored values are not exact measurements.
...
export async function GET(request: Request) {
  for (const cache of [globalRef.keywordApiCache, globalRef.singleAdCache]) {
    for (const [key, entry] of cache) if (Date.now() - entry.timestamp > CACHE_TTL_MS) cache.delete(key);
  ...
  try {
    ...
    const cacheKey = `v110_verified_${query.toLowerCase()}_${detectedCategoryAnchor || ''}`;
    ...
    const totalPosts = mainStats.totalPosts;
    const mainMonthlyPosts = mainStats.monthlyPosts;
    ...
    if (adRes?.data?.keywordList) {
      ...
      if (exactMatch) {
        ...
        isRealSearchAdData = Number.isFinite(totalSearchVolume);
      ...
      const chunkRes = await Promise.all(
        chunk.map(async (item) => {
          try {
            ...
            if (compRatio === null) {
              grade = 'UNKNOWN'; gradeLabel = '확인 불가';
            ...
            return {
              ...
              monthlyPostsIsLowerBound: stats.monthlyPostsIsLowerBound,
    ...
    const competitionRatio = isRealSearchAdData && mainStats.available && totalSearchVolume > 0 ? parseFloat((totalPosts / totalSearchVolume).toFixed(2)) : null;

    let grade: 'GOLD' | 'NORMAL' | 'HARD' | 'UNKNOWN';
    let statusText: string;

    if (competitionRatio === null) {
      grade = 'UNKNOWN'; statusText = '데이터 부족으로 경쟁도를 계산할 수 없습니다.';
    ...
    const responsePayload = {
      ...
      data: {
        ...
        mainKeyword: query,
        entityType,
        pcSearchVolume: isRealSearchAdData ? pcSearchVolume : null,
        mobileSearchVolume: isRealSearchAdData ? mobileSearchVolume : null,
        totalSearchVolume: isRealSearchAdData ? totalSearchVolume : null,
        ...
        monthlyPostsIsLowerBound: mainStats.monthlyPostsIsLowerBound,
